...
def get_sentence_spans(token_seq: List[str]) -> List[Tuple[int,int]]:
    """
    Given a list of tokens, returns (start,end) indices for each sentence.
    Splits at any token in SENTENCE_BOUNDARIES; if none found, returns one span.
    """
    spans = []
    start = 0
    for idx, tok in enumerate(token_seq):
        if tok in SENTENCE_BOUNDARIES:
            # include the boundary token in the sentence
            spans.append((start, idx+1))
            start = idx+1
    # anything remaining after the last boundary
    if start < len(token_seq):
        spans.append((start, len(token_seq)))
    # at least one span
    return spans or [(0, len(token_seq))]
...
def prepare_train_batch(pairs_to_batch, batch_size):
    ...
    for batch in batches:
        spans_for_batch = []
        for i, li, j, lj, num, num_pos, num_stack in batch:
            # note: 'i' is the padded input list, but we want the unpadded version:
            seq = i[:li]  
            spans_for_batch.append(get_sentence_spans(seq))
        sentence_spans_batches.append(spans_for_batch)
    return input_batches, input_lengths, output_batches, output_lengths, nums_batches, num_stack_batches, num_pos_batches, num_size_batches , sentence_spans_batches
