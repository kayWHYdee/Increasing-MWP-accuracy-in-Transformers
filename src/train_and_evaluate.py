...
from src.pre_data import get_sentence_spans
...
def train_tree(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, generate_nums,
               ...
               merge_optimizer, output_lang, num_pos, sentence_spans, english=False):
    ...
    seq_mask = torch.BoolTensor(seq_mask)
        ...
        # ——— ADAPTER PATCH ————
    # If our encoder is the RoBERTaAdapter, pad seq_mask out to its fixed max_length
    if hasattr(encoder, 'roberta') and hasattr(encoder.roberta, 'config'):
        B, orig_len = seq_mask.size()
        max_enc_len = encoder.roberta.max_length
        if orig_len < max_enc_len:
            # mask all those extra positions
            pad = torch.ones(B, max_enc_len - orig_len, dtype=torch.bool)
...
# after
    encoder_sequence_outputs, encoder_root_representation = encoder(
    input_var,
    input_length,
    sentence_spans             # <-- pass your sentence_spans here
)
    # Prepare input and output variables
    node_stacks = [[TreeNode(_)] for _ in encoder_root_representation.split(1, dim=0)]
    ...
    all_nums_encoder_outputs = get_all_number_encoder_outputs(encoder_sequence_outputs, num_pos, batch_size, num_size,
                                                              encoder.hidden_size)
    ...
    for t in range(max_target_length):
        p_leaf , num_score, op, current_embeddings, current_context, current_nums_embeddings = predict(
            node_stacks, left_childs, encoder_sequence_outputs, all_nums_encoder_outputs, encoder_root_representation, seq_mask, num_mask)
    ...
    op_target = (target < num_start).long()
    loss_0 = masked_cross_entropy_without_logit(all_leafs, op_target, target_length)
    loss_1 = masked_cross_entropy(all_node_outputs, target, target_length)
    loss   = loss_1 + loss_0                        # combine both
    if t == 0 and idx % 50 == 0:   # e.g. first time step, every 50th batch
        print(f"[train_tree] loss_0 (leaf) = {loss_0.item():.4f}, loss_1 (node) = {loss_1.item():.4f}")
...
#                   sentence_spans, beam_size=5, english=False, max_length=MAX_OUTPUT_LENGTH):

#     token_seq = [encoder.input_lang.index2word[idx] for idx in input_batch]
#     # sentence_spans = get_sentence_spans(token_seq)

#     single_spans = get_sentence_spans(token_seq)
#     # wrap in an outer list to make it List[List[Tuple[int,int]]] of length batch_size=1
#     sentence_spans = [ single_spans ]

#     seq_mask = torch.BoolTensor(1, input_length).fill_(0)
#     # Turn padded arrays into (batch_size x max_len) tensors, transpose into (max_len x batch_size)
#     if hasattr(encoder, 'roberta') and hasattr(encoder.roberta, 'max_length'):
#         B, orig_len = seq_mask.size()
#         max_enc_len = encoder.roberta.max_length
#         if orig_len < max_enc_len:
#             pad = torch.ones(B, max_enc_len - orig_len, dtype=torch.bool, device=seq_mask.device)
#             seq_mask = torch.cat([seq_mask, pad], dim=1)
...
#     input_var,
#     [input_length],
#     sentence_spans            # evaluates with same spans
# )
...
#             p_leaf , num_score, op, current_embeddings, current_context, current_nums_embeddings = predict(
#                 b.node_stack, left_childs, encoder_sequence_outputs, all_nums_encoder_outputs, encoder_root_representation,
#                 seq_mask, num_mask)
...
def evaluate_tree(input_batch, input_length, generate_nums,
                  ...
                  beam_size=5, english=False,
                  max_length=MAX_OUTPUT_LENGTH):

    # 1) Recompute fresh sentence spans
    token_seq = [encoder.input_lang.index2word[idx] for idx in input_batch]
    single_spans = get_sentence_spans(token_seq)
    sentence_spans = [single_spans]

    # 2) Build seq_mask (and pad for RoBERTa)
    seq_mask = torch.BoolTensor(1, input_length).fill_(0)
    if hasattr(encoder, 'roberta') and hasattr(encoder.roberta, 'max_length'):
        B, orig_len = seq_mask.size()
        max_enc_len = encoder.roberta.max_length
    ...
    # 3) Prepare input/num masks
    input_var = torch.LongTensor(input_batch).unsqueeze(1)
    num_mask  = torch.BoolTensor(1, len(num_pos) + len(generate_nums)).fill_(0)

    # 4) Switch to eval mode & move to GPU
    ...
    # 5) Run the encoder
    encoder_sequence_outputs, encoder_root = encoder(
        input_var, [input_length], sentence_spans
    )

    # 6) Initialize stacks & number encodings
    node_stacks = [[TreeNode(encoder_root[0].unsqueeze(0))]]
    all_nums_encoder_outputs = get_all_number_encoder_outputs(
        encoder_sequence_outputs, [num_pos], 1,
        len(num_pos), encoder.hidden_size
    )
    num_start = output_lang.num_start
    embedding_stacks = [[]]
    left_childs      = [None]

    # Helper to shallow-copy the stacks (no tensor deepcopy)
    def copy_stacks(stacks):
        return [list(s) for s in stacks]

    # Initial beam: score, node_stack, embed_stack, left_childs, output_ids
    beams = [TreeBeam(0.0, node_stacks, embedding_stacks, left_childs, [])]
    ...
    for _ in range(max_length):
        ...
        while beams:
            b = beams.pop()
            # If this beam is done, keep it
            if len(b.node_stack[0]) == 0:
                new_beams.append(b)
                continue

            # Predict p_leaf + op/num scores
            p_leaf, num_score, op_logits, cur_emb, cur_ctx, cur_nums = predict(
                b.node_stack, b.left_childs,
                encoder_sequence_outputs, all_nums_encoder_outputs,
                encoder_root, seq_mask, num_mask
            )
            ...
            if _ == 0 and len(beams)==1 and False:   # turn the False -> True for one debug run
                ...
                print("  raw p_leaf       :", p_leaf.data.cpu().numpy())
                print("  sample op_logits :", op_logits[0,:5].data.cpu().numpy(), "…")
            ...
            for tv, ti in zip(topv.split(1, dim=1), topi.split(1, dim=1)):
                ...
                # shallow-copy stacks
                ns = copy_stacks(b.node_stack)
                es = copy_stacks(b.embedding_stack)
                # we'll build new left_child for this beam
                lc = []

                out_ids = b.out + [tok]
                if tok < num_start:
                    # operator = leaf → do NOT push children
                    pass
                else:
                    # number = internal → expand
                    gen_in = torch.LongTensor([tok])
                    if USE_CUDA: gen_in = gen_in.cuda()
                    l_c, r_c, lbl = generate(cur_emb, gen_in, cur_ctx)
                    ns[0].append(TreeNode(r_c))
                    ns[0].append(TreeNode(l_c, left_flag=True))
                    es[0].append(TreeEmbedding(lbl[0].unsqueeze(0), False))

                # update left_childs for next step
                if es[0] and es[0][-1].terminal:
                    lc = [es[0][-1].embedding]
                else:
                    lc = [None]

                new_beams.append(
                    TreeBeam(new_score, ns, es, lc, out_ids)
                )

        # Keep top-K beams
        beams = sorted(new_beams, key=lambda b: b.score, reverse=True)[:beam_size]
        # Stop only when **every** beam is complete
        if all(len(b.node_stack[0]) == 0 for b in beams):
            break

    # Return the highest-scoring output sequence
    return beams[0].out
...
def topdown_train_tree(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch,
    ...
    encoder_sequence_outputs, encoder_root_representation = encoder(input_var, input_length)
    # Prepare input and output variables
    node_stacks = [[TreeNode(_)] for _ in encoder_root_representation.split(1, dim=0)]
